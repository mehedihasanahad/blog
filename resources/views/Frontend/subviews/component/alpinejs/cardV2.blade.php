<article class="w-full bg-white shadow-[1px_1px_10px_2px_rgba(0,0,0,0.1)] rounded-2xl p-6">
    ...
    <div class="col-span-2 overflow-hidden rounded-2xl">
        <a :href="'/blog/'+ blog.slug">
            <img class="w-full h-60 object-cover hover:scale-105 transition duration-500" :src="blog.featured_image">
        </a>
    </div>
    <div class="mt-5 col-span-3">
        {{--tag--}}
        <div class="flex gap-2">
            <template x-for="(tag,i) in blog.tags" key="tag.id">
                @includeIf('Frontend.subviews.component.alpinejs.bullet_tagV2')
            </template>
        </div>
        {{--title--}}
        <h1 class="mt-2 text-xl font-bold">
            <a :href="'/blog/'+ blog.slug" x-text="blog.title" class="decoration-pink-500 hover:underline underline-offset-4 decoration-2 overflow-ellipsis line-clamp-2"></a>
        </h1>
        {{--sub title--}}
        <p class="mt-2 overflow-ellipsis line-clamp-3" x-text="blog.subtitle"></p>
        {{--read time--}}
        <div class="mt-2 text-slate-400 flex gap-x-4">
            <time datetime="2022-05-02">
                ...
                <span x-text="new Date(blog.published_date).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric' })"></span>
            </time>
            <div x-show="blog.read_hour || blog.read_minute || blog.read_second">
                ...
                <span>
                    <span x-show="blog.read_hour" x-text="blog.read_hour + ' hr'"></span>
                    <span x-show="blog.read_minute" x-text="blog.read_minute + ' min'"></span>
                    <span x-show="blog.read_second" x-text="blog.read_second + ' sec'"></span>
                    read
                </span>
            </div>
        </div>
