            ...
            @foreach($letestblogs as $bIndex => $latestBlog)
                <article class="flex gap-x-1 mb-6">
                    ...
                    <h3 class="ml-2">
                        ...
                        <div>
                            @if(!empty($latestBlog->read_hour) || !empty($latestBlog->read_minute) || !empty($latestBlog->read_second))
                                <div>
                                ...
                                <span class="opacity-50">
                                    @if($latestBlog->read_hour)
                                        {{$latestBlog->read_hour}} hr
                                    @elseif($latestBlog->read_minute)
                                        {{$latestBlog->read_minute}} min
                                    @elseif($latestBlog->read_second)
                                        {{$latestBlog->read_second}} sec
                                    @endif
                                    read
                                </span>
                                </div>
